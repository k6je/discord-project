...
await Promise.all(tokens.map(async(token, i) => {
    ...
    client.on("ready", async() => {
        ...
        let channel;
        try { channel = client.channels.resolve(channel_id) } catch { return channel = null }
...
await Promise.all(tokens.map(async(token, i) => {
    ...
    client.on("ready", async() => {
        ...
        try { user = await client.users.fetch(user_id) } catch { return console.log(await replaceAll(lang.USER_NOT_FOUND, { clientUTAG: client.user.tag })) }
        if(!user) return console.log(await replaceAll(lang.USER_NOT_FOUND, { clientUTAG: client.user.tag }))

        if(client.relationships.cache.get(user.id) === 1) return console.log(await replaceAll(lang.USER_ALREADY_FRIEND, { clientUTAG: client.user.tag }));
        if(client.relationships.cache.get(user.id) === 4) return console.log(await replaceAll(lang.USER_ALREADY_SENT, { clientUTAG: client.user.tag }));
...
await Promise.all(tokens.map(async(token, i) => {
    ...
    client.on("ready", async() => {
        ...
        if(client.relationships.cache.get(user.id) !== 4) return console.log(await replaceAll(lang.USER_NOT_SENT, { clientUTAG: client.user.tag }));

        try {

        await wait(i * time * 1000);
        await client.relationships.cancelFriendRequest(user)
...
await Promise.all(tokens.map(async(token, i) => {
    ...
    client.on("ready", async() => {
        ...
        let inviteLink;
        try { inviteLink = await client.fetchInvite(invite) } catch (error) { return console.log(await replaceAll(lang.SERVER_NOT_FOUND, { clientUTAG: client.user.tag })) }
        if(!inviteLink || !inviteLink?.guild) return console.log(await replaceAll(lang.SERVER_NOT_FOUND, { clientUTAG: client.user.tag }))

        if(client.guilds.cache.get(inviteLink.guild.id)) return console.log(await replaceAll(lang.SERVER_ALREADY_JOINED, { clientUTAG: client.user.tag }));
